package SharedCodeBase.Huffman;
...
public class HuffmanCoding {
    ...
    public void printHuffmanCode(){
        for (String symbol : huffmanCode.keySet()) System.out.println(symbol + " > " + huffmanCode.get(symbol));
    }

    public String encode(String textToEncode){
        StringBuilder sb = new StringBuilder();
        for (int i = 0 ; i < textToEncode.length(); i++) sb.append(huffmanCode.get(textToEncode.substring(i, i + 1)));
        return sb.toString();
    }
    ...
    public String decode(String textToDecode){
        StringBuilder sb = new StringBuilder();
        int index = -1;
        while (index < textToDecode.length() - 2) index = findSymbol(root, index, textToDecode, sb);
        return sb.toString();
    }
    ...
    private void buildHuffmanTree(String text){
        ...
        for(int i = 0; i < text.length(); i++){
            ...
            if(temp == null)freqMap.put(text.charAt(i), new Node(text.substring(i, i+ 1), 1));
            else temp.freq += 1;
        }
    ...
    private void fillHuffmanCode(Node actualNode, String str, HashMap<String, String> huffmanCode) {
        ...
        if (actualNode.left == null && actualNode.right == null) huffmanCode.put(actualNode.symbol, str);
    ...
    private int findSymbol(Node actualNode, int index, String textToDecode, StringBuilder decodedText) {
        if (actualNode == null) return index;
        if (actualNode.left == null && actualNode.right == null) {
            decodedText.append(actualNode.symbol);
            return index;
        }
        index++;
        if (textToDecode.charAt(index) == '0') index = findSymbol(actualNode.left, index, textToDecode, decodedText);
        else index = findSymbol(actualNode.right, index, textToDecode, decodedText);
        return index;
    }
}
