...
public class ErrorReporter {
    ...
    private Stamp stamp;
    private String objectClassName, errorName, errorVariable, fieldName, lastType, type, value, version = "NaN";
    ...
    public String check() {
        return COLOR.PURPLE + "Initialized ErrorReporter (library version: " + Version.ver + ")" + COLOR.RESET;
    }
    ...
    public void objectError(Object objectIn, String errorVariableIn, Exception exception, Integer errorId) {
        ...
        memory = new ErrorReporterMemory(errorLogFile, null, arguments.get("development"));

        stamp = new Stamp(new Date().getTime());

        memory.appendLine(COLOR.RED + "--- " + objectClassName + " CRASHED: \"" + errorName + "\" ---" + COLOR.RESET);
        memory.appendLine(COLOR.CYAN + "- Crash date and time: " + Stamp.getDate() + " " + Stamp.getFormattedTimeNow() + COLOR.RESET);
        memory.appendLine(COLOR.CYAN + "- SharedCodeBase Version: " + Version.ver + COLOR.RESET);
        memory.appendLine("");
        ...
        if(public_fields.size() > 0) {
            memory.appendLine(COLOR.YELLOW + "--- " + objectClassName + " PUBLIC VARIABLES ---" + COLOR.RESET);
        ...
        memory.appendLine("");
        if (exception != null) memory.appendLine(COLOR.CYAN + "--- JAVA THROWN EXCEPTION ---\n" + exception.getCause() + exception.getMessage() + COLOR.RESET);

        finished_in = new Date().getTime() - stamp.start_time;

        memory.appendLine("");
        memory.appendLine("--- ErrorReporter finished in " + ((finished_in < 15) ? COLOR.GREEN : ((finished_in < 25) ? COLOR.YELLOW : ((finished_in < 50) ? COLOR.ORANGE : COLOR.RED))) + finished_in + "ms" + COLOR.RESET + " ---");
    ...
    private void printVariable(Field field) {
        try {
            ...
            if(!type.equals(lastType)) memory.appendLine(COLOR.CYAN + "\n--- " + (lastType = type) + " ---" + COLOR.RESET);
            ...
            memory.appendLine((fieldName.equals(errorVariable)?COLOR.RED:COLOR.BLUE) + "- " + fieldName + ": " + COLOR.RESET + COLOR.GREEN + value + COLOR.RESET);
        } catch (Exception e) {
